...
function initToDoList(jq) {
	...
	obj.src = 'https://cdn.jsdelivr.net/gh/cxvh/todo-list@main/main.js'
	document.querySelector('head').appendChild(obj)
	jq('[data-todolist]').addClass('lackkTodo').html(`<div id=TodoCon></div><div class=TodoAdd><textarea id=todo placeholder="To do list_"></textarea> <button id=add>Todo</button></div>`)
	...
	jq('#todo').blur(function(){
		jq(this).css('text-align', 'right')
	})
}
if (window.jQuery) {
	if(document.querySelector('[data-todolist]')){
